package by.it.group251051.birukov01.lesson11;
...
public class MyLinkedHashSet<E> implements Set<E>
{
    ...
    protected static class MyList<E>
	{
		...
		{
            LNode<E> curr = head;
            while (curr != null && !curr.data.equals(o)) curr = curr.next;
            return curr != null;
        }

        public boolean add(E o, int actPose, boolean toCheck)
		{
            if (toCheck && contains(o)) return false;
            LNode<E> curr = new LNode<E>(o, actPose);
            if (tail == null) head = tail = curr;
            else
			{
                tail.next = curr;
                tail = curr;
        ...
        public boolean remove(E o)
		{
            ...
            LNode<E> prev = head;
            while (prev.next != null && !prev.next.data.equals(o)) prev = prev.next;
    ...
    {
        for (int i = 0; i < DefaultSize; i++) map[i] = new MyList<E>();
    }
    ...
    public String toString()
	{
        ...
        for (int i = 0; i < DefaultSize; i++) nods[i] = (LNode<E>) map[i].head;
        while (true)
		{
	...
	{
        if (map[e.hashCode() % DefaultSize].add(e, lastPose++, true)) actSize++;
        else return false;
        return true;
    }
	...
	{
        if (map[o.hashCode()%DefaultSize].remove(o)) actSize--;
        else return false;
        return true;
    }
    ...
    public boolean removeAll(Collection<?> c)
	{
        ...
        for (int i = 0; i < DefaultSize; i++)
		{
            ...
            while (curr != null)
			{
                if (!c.contains(curr.data)) newMyList.add(curr.data, curr.pose, false);
                else deleted++;
                curr = curr.next;
            }
            map[i] = newMyList;
