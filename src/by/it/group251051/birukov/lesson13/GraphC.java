package by.it.group251051.birukov01.lesson13;
...
public class GraphC
{
    private static class vector<T>
	{
		...
		{
            if (size == values.length) values = Arrays.copyOf(values, size * 3 / 2 + 1);
            values[size++] = value;
        }
    ...
    public static void main(String[] args)
	{
        ...
        while (!stopinput)
		{
            ...
            a = a.substring(0, 1);
            if (c.charAt(c.length() - 1) != ',') stopinput = true;
            else c = c.substring(0, c.length() - 1);
        ...
        for(int i = 0; i < G.size; ++i)
            if(!hasUnit.containsKey(units[i]))
			{
                ...
                for(int j = i; j < G.size; ++j)
                    if(units[j] == units[i]) temp = temp + chars.get(j);
        ...
        for(int i = 0; i < ans.size; ++i) System.out.println((String)ans.values[i]);
        in.close();
    }
}