package by.it.group251051.birukov01.lesson12;
...
public class MyRbMap implements SortedMap<Integer, String>
{
    ...
    private void delete(Integer key)
	{
		...
		{
            if (deleted == head) head = null;
            else
			{
                if (deleted.parent.left == deleted) deleted.parent.left = null;
                else deleted.parent.right = null;
            }
            return;
        }
        Node swapped = null;
        ...
        else swapped = getChild(deleted);
        deleted.data = swapped.data;
        deleted.key = swapped.key;
        removeOneChild(swapped);
        if (isBlack(swapped) || isBlack(deleted)) balanceDelete(getChild(swapped));
        head.color = colors.BLACK;
    }
    ...
    private void balanceDelete(Node ch)
	{
        while (ch != head && ch != null)
		{
            if (ch.parent.left == ch)
			{
                ...
                if (isBlack(b.right) && isBlack(b.left)) b.color = colors.RED;
                else
				{
                    if (isBlack(b.right))
					{
    ...
    public String toString()
	{
        ...
        toStr(head, res);
        if (head != null) res = new StringBuilder(res.substring(0, res.length() - 2));
        res.append("}");
    ...
    public void fillLower(Node n, Integer key)
	{
        ...
        if (n.key < key) this.put(n.key, n.data);
    ...
    private void recGetByValue(Node n, Object data)
	{
        ...
        if (n.data.equals(data)) ContainsValueF = true;
    ...
    private Node recGet(Node n, Integer key)
	{
        ...
        else if (key < n.key) return recGet(n.left, key);
        else if (key > n.key) return recGet(n.right, key);
        else return n;
    }
