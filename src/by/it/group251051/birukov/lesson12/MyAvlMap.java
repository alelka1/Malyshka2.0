...
public class MyAvlMap implements Map<Integer, String>
{
    private class Node
	{
        ...
        public Node balance()
		{
			...
			{
                if (getBFactor(this.right) < 0) this.right = this.right.rotateRight();
                return rotateLeft();
            }
            if (getBFactor(this) == -2)
			{
                if (getBFactor(this.left) > 0) this.left = this.left.rotateLeft();
                return rotateRight();
            }
            return this;
        }
    ...
    private Node recInsert(Node n, Integer key, String data)
	{
        ...
        if (key < n.key) n.left = recInsert(n.left, key, data);
        else n.right = recInsert(n.right, key, data);
        return n.balance();
    }
	...
	{
        if (n == null) return null;
        if ((Integer) key < n.key) n.left = recRemove(n.left, key);
        else if ((Integer) key > n.key) n.right = recRemove(n.right, key);
        else
		{
            Node l = n.left;
            Node r = n.right;
    ...
    public String toString()
	{
        ...
        toStr(tree, res);
        if (tree != null) res = new StringBuilder(res.substring(0, res.length() - 2));
        res.append("}");
