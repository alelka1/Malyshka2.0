...
public class MySplayMap implements NavigableMap<Integer, String>
{
    ...
    private Node recGet(Node n, Integer key)
	{
        ...
        if (n.key == key) return splay(n);
        if (key < n.key && n.left != null) return recGet(n.left, key);
        if (key > n.key && n.right != null) return recGet(n.right, key);
        return splay(n);
    }
	...
	{
        if (root == null) return new SplitNode(null, null);
    ...
    private void recGetLower(Node n, Integer key)
	{
        ...
        if (FMax == null && n.key < key) FMax = n.key;
        else if (n.key < key && FMax < n.key) FMax = n.key;
    ...
    private void recGetCeiling(Node n, Integer key)
	{
        ...
        if (FMin == null && n.key >= key) FMin = n.key;
        else if (n.key >= key && FMin > n.key) FMin = n.key;
	...
	{
        head = recGet(head, (Integer) key);
        if (key.equals(head.key)) return head.data;
        return null;
    }
