package by.it.group251051.birukov01.lesson14;
...
public class StatesHanoiTowerC
{
    ...
    public static void main(String[] args)
	{
        ...
        for (int i = 0; i < max_size; i++) {
            int step = i + 1;
            int[] delta;
            if (step % 2 != 0) delta = carryingOver(n, step, 1);
            else
			{
                int count = step;
                int countDisk = 0;
            ...
            int max = max(heights);
            if (steps_heights[max - 1] == -1) steps_heights[max - 1] = i;
            else dsu.union(steps_heights[max - 1], i);
        }
        int[] sizes = new int[n];
        for (int i = 0; i < n; i++)
