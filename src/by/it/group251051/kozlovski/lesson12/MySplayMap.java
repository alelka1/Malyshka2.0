...
public class MySplayMap implements NavigableMap<Integer, String> {
    ...
    private void traverseToString(MySplayNode p, Str res){
        if(p == null) return;
        if(p.left != null)
    ...
    private MySplayNode getParent(MySplayNode t){
        return t.parent;
    }

    private MySplayNode getGrandparent(MySplayNode t){
        MySplayNode p = getParent(t);
        return p != null ? getParent(p) : null;
    }

    private void leftRotate(MySplayNode a){
        MySplayNode l = a.left, b = a.right;
        if(b != null){
            MySplayNode c = b.left, r = b.right;
            b.parent = a.parent;
            if(b.parent != null){
                if(b.parent.left == a)
                    b.parent.left = b;
    ...
    private void rightRotate(MySplayNode a){
        MySplayNode b = a.left, r = a.right;
        if(b != null){
            MySplayNode l = b.left, c = b.right;
            b.parent = a.parent;
            if(b.parent != null){
                if(b.parent.left == a)
                    b.parent.left = b;
    ...
    private void Zig(MySplayNode a){
        if(a.parent.left == a)
    ...
    private void ZigZig(MySplayNode x){
        MySplayNode g = getGrandparent(x), p = getParent(x);
        if(p.left == x){
            rightRotate(g);
            rightRotate(p);
        } else {
    ...
    private void ZigZag(MySplayNode x){
        MySplayNode g = getGrandparent(x), p = getParent(x);
        if(p.right == x){
            leftRotate(p);
            rightRotate(g);
    ...
    private void Splay(MySplayNode x){
        while (root != x) {
            MySplayNode p = getParent(x);
            if (p == root)
                Zig(x);
            else {
                MySplayNode g = getGrandparent(x);
    ...
    private MySplayNode FindKey(Object key){
        MySplayNode t = root;
    ...
    private MySplayNode FindFirstGreater(MySplayNode t){
        t = t.right;
        while(t.left != null)
            t = t.left;
        return t;
    ...
    private MySplayNode putKey(Integer key){
        MySplayNode prev = root, t = prev;
        while(t != null){
            prev = t;
            if(key < t.key)
                t = t.left;
    ...
    private void removeLeaveNode(MySplayNode x){
        if(x.parent != null) {
            if (x.parent.left == x)
                x.parent.left = null;
            else
    ...
    private void removeNode(MySplayNode exist){
        if(exist.left == null && exist.right == null) {
            MySplayNode p = getParent(exist);
            ...
            MySplayNode del = FindFirstGreater(exist);
    ...
    private Integer travelLowerKey(MySplayNode p, Integer key, boolean inclusion){
    ...
    private Integer travelHigherKey(MySplayNode p, Integer key, boolean inclusion){
        Integer ans = (p.key > key) || (inclusion && p.key.equals(key)) ? p.key : null;
    ...
    private void traverseHeadMap(MySplayNode p, NavigableMap<Integer, String> res, Integer toKey){
        if(p != null) {
            if (p.left != null)
    ...
    private void traverseTailMap(MySplayNode p, NavigableMap<Integer, String> res, Integer fromKey){
        if(p == null) return;
        if(p.left != null)
